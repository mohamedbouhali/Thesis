...
class HGGLearner:
	...
	def learn(self, args, env, env_test, agent, buffer):
		"""
		Returns list of explored goals after each episodes.
		"""
		initial_goals = []
		desired_goals = []

		#action_list added by mohamed to get a list of all action taken by the agent in te the training
		action_list = []
		for i in range(args.episodes):
		...
		for i in range(args.episodes):
			...
			self.explored_goals.append(explore_goal)
